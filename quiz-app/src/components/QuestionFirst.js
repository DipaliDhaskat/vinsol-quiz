...
const QuestionFirst = (props) => {
    const totalQuestion = props.totalQuestion;
    const allQuizData = useSelector((state) => state.Reducer.allDataQuizData);
    const [flag, setFlag] = useState(1);
    let nextFlag = 1;
    const [timer, setTimer] = useState('00:00:00');
    const [firstNo, setFirstNo] = useState(allQuizData?.length > 0 ? allQuizData?.[allQuizData.length - 1]?.firstNumber : 0);
    const [secondNo, setSecondNo] = useState(allQuizData?.length > 0 ? allQuizData?.[allQuizData.length - 1]?.secondNumber : 0);
    const [operator, setOperator] = useState(allQuizData?.length > 0 ? allQuizData?.[allQuizData.length - 1]?.operatorRam : "+");
    const [question, setQuestion] = useState(allQuizData?.length > 0 ? allQuizData?.[allQuizData.length - 1]?.no : 0);
    const [userAnswer, setUserAnswer] = useState();
    const [allQuestion, setAllQuestion] = useState([]);
    const [answerRight, setAnswerRight] = useState(0);
    const [operands, setOperands] = useState(props.numRange);


    const Ref = useRef(null);
    const arrOperators = props.operator;
    const dispatch = useDispatch();


    useEffect(() => {
        allQuizData.length > 0 && setQuestion(allQuizData[allQuizData.length - 1]["no"]);
        handleNext();
    }, [])
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        question > 1 && dispatch(AllDataQuiz({ "no": question - 1, "question": ques, "userAnswer": userAnswer, "answer": ans, "firstNumber": firstNo, "secondNumber": secondNo, "operatorRam": operator }));
    ...
    return (
        <div >
            ...
            {
                flag === 4 && < FinalScore finalAnswer={answerRight} data="one" />
            }
        </div>
